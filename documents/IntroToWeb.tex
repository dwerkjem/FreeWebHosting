...
% Arrows
\draw[arrow] (user) -- node[above]{1. Request Page} (internet);
\draw[arrow] (internet) -- node[above]{2. Forward Request} (server);
\draw[arrow] (server) -- node[right]{3. Fetch Files} (files);

% Return arrow from Files to Internet (left of Files to right of Internet)
\coordinate (p1) at ($(files.west)+(-2.2,0)$);
\coordinate (p2) at ($(internet.east)+(2.2,0)$);
\draw[arrow] (files.west) -- (p1) |- (p2) -- (internet.east);
\node[align=center, anchor=west, font=\small\sf] at ($(p2)+(-1.4,-2.2)$) {4. Send Files};

\draw[arrow] (internet) -- node[below]{5. Deliver Page} (user);
